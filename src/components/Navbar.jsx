...
const Navbar = () => {
  ...
  // ✅ Debug: console me dekho user aa raha hai ya nahi
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            <img src="/lost-and-foundlogo.png" alt="Back2u Logo" className="w-10 h-10 rounded-full object-cover" />
            <Link to="/"><h1 className="text-xl font-bold bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent">Back2u</h1></Link>
          </div>
          ...
          <ul className="hidden md:flex items-center gap-8">
            ...
            {user && (
              <>
                ...
                <li><Link to="/user/my-claims" className="flex items-center gap-2 text-gray-300 hover:text-blue-400"><FaFileAlt /> My Claims</Link></li>
              </>
            )}
          </ul>
          ...
          <div className="hidden md:flex items-center gap-3">
            {user ? (
              <>
                {role === "admin" && (
                  <button onClick={() => handleNavigate("/admin/dashboard")} className="px-4 py-2 bg-yellow-600 hover:bg-yellow-700 text-white font-semibold rounded-lg transition duration-300 transform hover:scale-105">
                    Admin Panel
                  </button>
                )}
                <button onClick={handleLogout} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg transition duration-300 transform hover:scale-105">
                  Logout
                </button>
              </>
            ) : (
      ...
      {isOpen && (
        <div className="md:hidden bg-gray-900 border-t border-gray-700">
          <div className="px-4 py-4 space-y-2">
            ...
            {/* ✅ Sirf logged in user ko dikhenge */}
            {user && (
              <>
                <Link
                  to="/report"
                  onClick={closeMenu}
                  className="flex items-center gap-3 px-4 py-3 text-gray-300 hover:text-blue-400"
                >
                  <FaClipboardList /> Report Item
                </Link>
                <Link
                  to="/user/my-claims"
                  onClick={closeMenu}
                  className="flex items-center gap-3 px-4 py-3 text-gray-300 hover:text-blue-400"
                >
                  {/* ✅ FcDisclaimer hataya — FaFileAlt lagaya jo reliable hai */}
                  <FaFileAlt /> My Claims
                </Link>
              </>
            )}
            ...
            <div className="border-t border-gray-700 pt-3 mt-3 space-y-2">
              {user ? (
                <>
                  {role === "admin" && (
                    <button onClick={() => handleNavigate("/admin/dashboard")} className="w-full px-4 py-2 bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg">
                      Admin Panel
                    </button>
                  )}
